    ...
    <>
      ...
      >
        ...
        <div className="px-6 pt-6 pb-2 shrink-0 border-b border-border/40">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[9px] text-gray-400 font-medium uppercase tracking-wider">HR Admin Portal</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 shrink-0">
          <div className="flex items-center gap-4 p-4 rounded-2xl bg-gradient-to-br from-gray-50 to-white border border-border/60 shadow-sm">
            ...
            <div className="overflow-hidden">
              <h2 className="text-sm font-bold text-text-main truncate">{user?.username || 'HR Admin'}</h2>
              <p className="text-xs text-primary font-bold uppercase tracking-wider">{user?.role || 'Admin'}</p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-4 space-y-1.5 custom-scrollbar pb-4">
          ...
          {[
            { label: 'Dashboard', icon: <LayoutDashboard size={20} />, path: '/hrhome' },
            { label: 'Employees', icon: <Users size={20} />, path: '/hremployees' },
            { label: 'Add Employee', icon: <UserPlus size={20} />, path: '/hraddemployee' },
            { label: 'Payroll System', icon: <Wallet size={20} />, path: '/hrpayrollsystem' },
            { label: 'Calendar', icon: <Calendar size={20} />, path: '/hrcalendar' },
            { label: 'Profile', icon: <User size={20} />, path: `/hrprofile/${user?.id}` },
            { label: 'Leave Requests', icon: <NotepadText size={18} />, path: '/hrleaverequests' },
            // { label: 'Settings', icon: <Settings size={20} />, path: '/settings' },
            // { label: 'Employee Queries', icon: <UserRoundPen size={20} />, path: '/hrquery' },
          ].map(({ label, icon, path }) => {
